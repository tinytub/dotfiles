...
function M.rename_file()
  ...
  local old = assert(LazyUtil.root.realpath(vim.api.nvim_buf_get_name(buf)))
  local root = assert(LazyUtil.root.realpath(LazyUtil.root.get({ normalize = true })))
  ...
  M.on_supports_method("textDocument/documentHighlight", function(_, buf)
    vim.api.nvim_create_autocmd({ "CursorHold", "CursorHoldI", "CursorMoved", "CursorMovedI" }, {
      ...
      callback = function(ev)
        if not require("lsp.keymaps").has(buf, "documentHighlight") then
          return false
        end
