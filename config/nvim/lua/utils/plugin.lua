...
function M.fix_imports()
  Plugin.Spec.import = LazyUtil.inject.args(Plugin.Spec.import, function(_, spec)
    local dep = M.deprecated_extras[spec and spec.import]
    if dep then
      dep = dep .. "\n" .. "Please remove the extra to hide this warning."
      LazyUtil.warn(dep, { title = "LazyVim", once = true, stacktrace = true, stacklevel = 6 })
      return false
    end
  end)
end
...
function M.fix_renames()
  Plugin.Spec.add = LazyUtil.inject.args(Plugin.Spec.add, function(self, plugin)
