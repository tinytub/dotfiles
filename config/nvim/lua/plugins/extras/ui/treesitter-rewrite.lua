...
if vim.tbl_contains(Config.json.data.extras, "plugins.extras.ui.treesitter-rewrite") then
  if vim.fn.executable("tree-sitter") == 0 then
    LazyUtil.error("**treesitter-rewrite** requires the `tree-sitter` executable to be installed")
    return {}
  end

  if vim.fn.has("nvim-0.10") == 0 then
    LazyUtil.error("**treesitter-rewrite** requires Neovim >= 0.10")
    return {}
  end
end
...
return {
  {
    ...
    config = function(_, opts)
      ...
      opts.ensure_install = LazyUtil.dedup(vim.list_extend(norm(opts.ensure_install), norm(opts.ensure_installed)))
