return {
  -- Add java to treesitter.
  {
    ...
    opts = function(_, opts)
      if type(opts.ensure_installed) == "table" then vim.list_extend(opts.ensure_installed, { "java" }) end
    end,
  },
          ...
          vim.api.nvim_create_autocmd("FileType", {
            ...
            callback = function()
              ...
              local jdtls_base_config = {
                on_attach = require("utils").on_attach(function(client, buffer)
                  if mason_registry.has_package "java-test" then
                    -- custom keymaps for Java test runner (not yet compatible with neotest)
                    vim.keymap.set(
                      "n",
                      "<leader>tT",
                      function() require("jdtls").pick_test { bufnr = buffer } end,
                      { buffer = buffer, desc = "Run specific Test" }
                    )
                    vim.keymap.set(
                      "n",
                      "<leader>tt",
                      function() require("jdtls").test_class { bufnr = buffer } end,
              ...
              local jdtls_opts = require("utils").opts "nvim-jdtls"
