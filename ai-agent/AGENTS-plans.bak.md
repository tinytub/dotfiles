...
### 1.10 工具与证据（Tooling & Evidence）

* **先证据，后结论**：涉及代码行为、配置、日志、性能、线上现象的判断，优先基于可引用证据（文件片段、日志片段、命令输出、文档链接）再下结论；不确定时明确标注假设与验证点。
* **先读再改**：在进入 Plan（尤其是 moderate / complex）前，先阅读相关代码/配置/已有文档，避免“凭感觉改”。（与你在第 5.2 节中的要求一致）
* **工具使用最小化但不省略**：能用一次命令/一次查询验证的，不要靠猜；但避免无意义地“跑一堆命令”。每个命令都要对应一个明确问题。
* **引用格式**：给出关键证据位置（文件路径 + 关键行/段落 + 简短结论），或给出关键链接（不堆链接，只保留支撑结论的 3–5 个）。
...
* **例外（仅限必要场景）**：当任务明确涉及用户可见文案（UI 文案 / i18n）、需要与线上日志/报错原文保持一致、或用户明确要求输出中文文本时，允许在**字符串字面量**中出现中文；除此之外仍保持 English-only（尤其是标识符、注释、代码块内的说明文字）。
...
> **补充：小探针例外**  
> 在 Plan 模式末尾，允许附带**极小且可逆**的验证动作（例如 1–2 条探针命令、一个极小 PoC 片段）用于验证关键假设；但不得在 Plan 模式展开主要实现。  
> 一旦进入 Code 模式，主体必须是实现与交付（遵循第 5.4 节要求）。
...
### 5.5 Code 交付 DoD（每次必带）

* ✅ **变更点**：改了哪些文件/函数/模块，各自目的是什么
* ✅ **验证方式**：如何本地/CI/线上验证（具体命令或步骤 + 预期现象）
* ✅ **风险与回滚**（如适用）：可能的副作用、如何快速回退
* ✅ **未覆盖项 / TODO**：哪些边界情况未覆盖、后续怎么补
...
## 10 · 持久化规划（planning-with-files）

当任务属于「中等/复杂」或满足以下任一条件时，优先使用 Codex skill：`$planning-with-files`（若可用）来管理计划与过程记录，避免目标漂移与上下文丢失：
* 需要分 3 步以上才能完成
* 需要查资料/对比方案/反复试验
* 需要多次工具调用（例如多次搜索、跑脚本、改多处文件）
* 输出物较大（文档/方案/脚本/PR 级别改动）

工作方式（文件约定）：
* `task_plan.md`：作为唯一真相（source of truth）。写清目标、阶段划分、检查清单（checkbox），并在推进过程中持续勾选/更新。
* `notes.md`：记录调研资料、链接、关键发现、决策依据、遇到的问题与排查过程。
* `deliverable.md`（或任务专用交付文件名）：最终交付物（结论/方案/最终稿/最终脚本等）。

执行规则：
* 在做任何重大决定、从 Plan 切换到 Code、或准备输出最终结果之前，必须先回读并更新 `task_plan.md`。
* 任何临时想法、旁路结论、证据链，都进入 `notes.md`，不要只留在对话里。
* 每完成一个阶段，就在 `task_plan.md` 里更新进度与下一步。

与本 AGENTS.md 的 Plan/Code 工作流映射：
* Plan 阶段产物 → 写入 `task_plan.md`（必要时补充 `notes.md`）
* Code 阶段产物 → 写入交付文件（如 `deliverable.md`），并同步更新 `task_plan.md` 的 checkbox 与状态
